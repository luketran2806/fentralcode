...
const scrapeProduct = async (element) => {
  ...
  const productURL = `https://us.maje.com${productURLfound}`;
  colorname_details = [
    unique_color_id,
    productURL,
    swatchimgsrc,
...
async function main() {
  ...
  await page.goto(
    "https://us.maje.com/en/sale-see-all/?prefn1=smcp_subFamily&prefv1=Dresses%7CTops%20%26%20T-shirts%7CCoats%20%26%20Jackets%7CPants%20%26%20Jeans%7CJumpsuits%7CSkirts%20%26%20Shorts%7CSweaters%20%26%20Cardigans&start=0&sz=120",
  ...
  for (const product of allResults) {
    ...
    for (const color in product.datacolordetails) {
      ...
      const rowData = `Maje,${productName},${color},"${productInfo[1]}",${productInfo[0]},${productInfo[2]},"${productimgurl}"\n`;
      csvData += rowData;
    }
  }

  fs.writeFileSync("maje_product_data.csv", csvData, "utf8");
